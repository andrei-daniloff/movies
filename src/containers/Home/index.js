...
class Home extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      (genre !== params.genre || parsedSearch.page !== currentPage) &&
      !parsedSearch.id &&
      !search
    ) {
      this.setState(
        { loading: true, genre: this.props.match.params.genre, currentPage: parsedSearch.page },
        () => this.fetchMovies()
      );
    }
    ...
    if (
      (search && genre !== params.request) ||
      (search && prevParsedSearch.page !== parsedSearch.page)
    ) {
      this.setState(
        { loading: true, genre: this.props.match.params.request, currentPage: parsedSearch.page },
        () => this.fetchMovies()
      );
    }
  }
  ...
  fetchMovies = (id = null) => {
    ...
    if (!parsedSearch.id && !search) {
      axios
        .get(`${API}/movie/${genreLower}?${KEY}&language=en-US&page=${currentPage}`)
        .then(res =>
          this.setState({
            error: null,
            loading: false,
